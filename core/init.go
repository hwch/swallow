...
func init() {
	...
	g_builtin.set("print", NewFunc(true, nil, "print", &Param{flag: -1}, nil, nil))
	g_builtin.set("list", NewFunc(true, nil, "list", &Param{flag: -1}, nil, nil))
	g_builtin.set("exit", NewFunc(true, nil, "exit", &Param{flag: 0}, nil, nil))
	...
	cls_init_func := &Func{scope: inScope,
		...
		isBuiltin: false,
		params:    &Param{flag: 0},
		body: &LocalCompoundStatement{scope: inScope,
